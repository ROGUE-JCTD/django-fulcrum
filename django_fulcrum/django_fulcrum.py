...
import os
from .models import Asset, get_type_extension, Feature, Changesets
...
class DjangoFulcrum:
        ...
        for grouped_features in chunks(imported_features, upload_to_geogig, time_field=time_field, time_delta=2):
            ...
            if filtered_features:
                ...
                for feature in filtered_features.get('features'):
                    ...
                    # if feature.get('properties').get('changeset_id') and
                    # feature.get('properties').get('changeset_id') in changeset_dict:
    ...
    def get_latest_records(self, layer):
        ...
        records = []
        if layer.layer_date and layer.layer_date != 0:
...
def write_changesets_to_db(changesets_list, form_id, geojson=False):
    ...
    for changeset in changesets_list:
        ...
        if geojson:
            uid = changeset.get('fulcrum_id')
            comment = changeset.get('comment')
        else:
            uid = changeset.get('id')
            if changeset.get('metadata').get('comment'):
                comment = changeset.get('metadata').get('comment')
            else:
                comment = ""
        Changesets.objects.get_or_create(changeset_uid=uid, changeset_form_id=form_id, changeset_created_at=created_at,
                                         changeset_updated_at=updated_at, changeset_number_of_changes=change_count,
...
def get_features_by_changeset(changeset, layer_name):
    """
    ...
    Args:
         ...
         layer_name: The name of the layer used to query features
    Returns:
        A queryset of all features with the specified changeset_id
    """
    layer = Layer.objects.get(layer_name=layer_name)
    try:
        feature_models = Feature.objects.filter(feature_changeset=changeset, layer=layer)
        features = []
...
def changeset_chunks(form_id, layer_name):
    """
    Args:
        form_id: The name of the layer from which features will be yielded
        layer_name: the layer name of the features which will be yielded
    Returns:
    ...
    if changesets.exists():
        for changeset in changesets:
            features = get_features_by_changeset(changeset, layer_name)
            if not features:
                continue
